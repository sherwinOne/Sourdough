...
class YaGsonConverterFactory : Converter.Factory() {
    ...
    override fun responseBodyConverter(type: Type, annotations: Array<out Annotation>, retrofit: Retrofit): Converter<ResponseBody, Any> {
        val adapter = Json.gson.getAdapter(TypeToken.get(ApiModel::class.typeWith(type)))
        return Converter { value ->
            value.use {
                @Suppress("UNCHECKED_CAST")
                val responseData = (adapter.fromJson(it.charStream()) as ApiModel<Any>)
